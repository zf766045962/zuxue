...
class smtp_mailer extends xwbMailer {
	...
	function sendEmails($toList, $subject='s', $body=''){
		...
		$from = array(SYS_SMTP_FROM,SYS_SMTP_TITLE);
		...
		$this->setSMTP(SYS_SMTP_HOST, SYS_SMTP_USER, SYS_SMTP_PASS, SYS_SMTP_PORT);
...
class xwbMailer {
   ...
   * @var string
   */
	var $From              = 'xtalk_service@sina.com';
   ...
   */
	...
	function setSMTP($host, $user, $pwd, $port=25){
		$this->Host			= $host;
		$this->Username		= $user;
		$this->Password		= $pwd;
		$this->Port			= $port;
   ...
   * @return bool
   */
	function Send($from, $recipients, $subject, $body, $options = '') {
		//处理发件者
		if (is_array($from)) {
			//特殊处理
			$this->From = $from[0];
		...
		//处理options
		if (is_array($options)) {
			if (!empty($options['out_charset'])) {
				$this->CharSet = $options['out_charset'];
			}
   ...
   */
	function SmtpConnect() {
		if($this->smtp == NULL) {
			$this->smtp = new smtp_mailer_ctrl();
			//$this->smtp = APP::ADP('mailer');
		}
		...
		while($index < count($hosts) && $connection == false) {
			$hostinfo = array();
			if(preg_match('#^(.+):([0-9]+)$#sim', $hosts[$index], $hostinfo)) {
				$host = $hostinfo[1];
				$port = $hostinfo[2];
			} else {
				$host = $hosts[$index];
