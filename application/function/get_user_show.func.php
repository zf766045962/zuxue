	...
	if ($countId > 0) {
		...
		if ($countId > 20) {
			...
			for ($p=1; $p <=$pageCnt; $p++) {
				...
				$rspTmp = $isName ? DR('Xsmart/xwb.getUsersBatchShow', false, false, implode(',', $idsTmp)) : DR('Xsmart/xwb.getUsersBatchShow', '', implode(',', $idsTmp));
			...
			$result = $isName ? DR('Xsmart/xwb.getUsersBatchShow', $cache, false, $ids) : DR('Xsmart/xwb.getUsersBatchShow', $cache, $ids);
		}
	}
	return $result;
}
