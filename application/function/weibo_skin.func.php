...
function selectSkin($skinCssDirName, $skin_id, $skin_list, $route)
{
	if (!$skinCssDirName){
		switch($route['class']) {
			...
			case 'ta':
				if(trim(V('g:id',0))) {
					$cc_uid = trim(V('g:id',0));
				}else{
					$user_info = DR('Xsmart/xwb.getUserShow', '', null, null, urldecode(V('g:name','')), false);
					if($user_info['rst']) {
						$cc_uid = $user_info['rst']['id'];
					}
				}
				if(isset($cc_uid)) {
					$skin_rst = DR('common/userConfig.get', '', 'user_skin', $cc_uid);
			...
			case 'show':
				...
				$id = trim(V('g:id',0));
				if(isset($id)) {
					$mblog_rst = DR('Xsmart/xwb.getStatuseShow', '', $id);
		...
		switch($router['class']){
			case 'show':
				...
				if(!$blogid&&!USER::uid()){
					...
					$mblog_rst = DR('Xsmart/xwb.getStatuseShow', '', $blogid);
					if(isset($mblog_rst['rst']['errno'])&&$mblog_rst['rst']['errno']==0){
						
						$cc_uid=$mblog_rst['rst']['user']['id'];
					}
					else{
						$useDefault=TRUE;
					}					
			...
			case 'ta':
			default:
				$cc_uid=trim(V('g:id'),FALSE);	
		}
	}
	elseif(USER::isUserLogin()){
		$cc_uid=USER::uid();
	}
	if(isset($cc_uid)&&$cc_uid){
		// 这里获取的可能是自己或者别人的皮肤，所以没有用V
		$skin_rst = DR('common/userConfig.get', 'u0/0', 'user_skin', $cc_uid);
		...
		elseif(is_numeric($skin_rst['rst'])){
			...
			//自定义皮肤
			return array('path'=>URL('setting.getSkin','id='.$cc_uid));
