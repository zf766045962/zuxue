	...
	function uploadLogo() {
		if ($this->_isPost()) {
			...
			while ($file && $file['tmp_name']) {
				...
				$file_arr = explode('.', IMG_LOGO_PREVIEW_FILE_NAME);
				if (!$file_obj->upload('logo', array_shift($file_arr), P_VAR_NAME, 'png')) {
					$state = '复制文件时出错,上传失败' . $file['tmp_name'] . '===' . P_VAR . IMG_LOGO_PREVIEW_FILE_NAME;
					break;
				}
				//获取上传文件的信息
	...
	function editIndex() {
		if ($this->_isPost()) {
			...
			$data = array(
				...
				'site_keyword' 					=> V('p:site_keyword', ''),							//网站第三方统计代码
				'site_meat' 					=> V('p:site_meat', ''),							//网站第三方统计代码
				'site_contact' 					=> V('p:site_contact', ''),							//网站第三方统计代码
				'site_tel' 						=> V('p:site_tel', ''),							//网站第三方统计代码
				'site_learn'					=> V('p:site_learn',''),								//学币
				'site_money'					=> V('p:site_money',''),								//金额
...
//				'wb_lang_type' 					=> trim( V('p:wb_lang_type', 'zh_cn') ),			//网站当前语言
				...
				'logo_link'						=> V('p:logo_link', '1')							// Logo链接为我的首页或广场,1为广场,2为我的首页
