...
class celeb_mgr_mod extends action
{
		...
		if ($this->_isPost()) {
			...
			if ($file) {
				...
				if(XWB_SERVER_ENV_TYPE == "sae") {
					$file_arr = explode('.', WB_CELEB_PREVIEW_FILE_NAME);
					$file_content = IO::read(array_shift($file_arr));
					$url = IO::write(P_VAR . WB_CELEB_BANNER_FILE_NAME, $file_content);
					
					$banner = $url;
				}elseif(is_file(P_VAR . WB_CELEB_PREVIEW_FILE_NAME)) {
					$file_content = IO::read(P_VAR . WB_CELEB_PREVIEW_FILE_NAME);
					IO::write(P_VAR . WB_CELEB_BANNER_FILE_NAME, $file_content);
					//小图处理
					$banner = P_VAR_NAME .WB_CELEB_BANNER_FILE_NAME;
				}
				
			}
			$result = DR('common/sysConfig.set', '', 'celeb_banner', $banner);
			if($result['errno']) {
				$this->_error('配置失败',  array('editIndex'));
			}
			$this->_succ('已经成功保存你的配置', array('setting'));
		}
		$d = DR('common/sysConfig.get');
		if ($d['rst']) {
			TPL::assign('config', $d['rst']);
		}
		$this->_display('celeb/celebMgr_setting');
	}
	...
	function uploadBanner() {
		if ($this->_isPost()) {
			...
			while ($file && $file['tmp_name']) {
				...
				if ($info[0] > 758 || $info[1] > 120) {
					$state = '上传的图片长宽(758x120)超过限制，请重新选择';
					break;
				}
				...
				$file_arr = explode('.', WB_CELEB_PREVIEW_FILE_NAME);
				if (!$file_obj->upload('file', array_shift($file_arr), P_VAR_NAME, 'jpg,jpeg,gif,png')) {
					$state = '复制文件时出错,上传失败' . $file['tmp_name'] . '===' . P_VAR . WB_CELEB_PREVIEW_FILE_NAME;
					break;
				}
				//获取上传文件的信息
	...
	function addStar()
	{
		...
		if ($c_id1 && $c_id2 && $cc_uid) {
			$rs = DR('Celeb.getUserByInfo', '', array('c_id1'=>$c_id1, 'c_id2'=>$c_id2, 'cc_uid'=>$cc_uid));
			if ($rs['rst']) {
				$info = $rs['rst'][0];
			}
		}
	...
	function saveStar()
	{
		...
		if ($nick) {
			...
			//查询API接口，获取用户信息
//			DR('Xsmart/xwb.setSchoolAppKey', '', 1);   //设置API TOKEN
			$user_info = DR('Xsmart/xwb.getUserShow', '', null, null, $nick);
...
//            	$this->_log('添加名人堂用户：data=' . serialize($data), 'add');
			...
			if (empty($old_c_id1) || empty($old_c_id2) || empty($cc_uid)) {
				$this->_error('参数错误！', URL('mgr/celeb_mgr.starList'));
			}
			...
			if ($old_c_id1 !== $c_id1 || $old_c_id2 !== $c_id2) {
				//检测用户是否已添加到相应的分类当中
				$crs = DR('Celeb.getUserByInfo', '', array('c_id1' => $c_id1, 'c_id2' => $c_id2, 'cc_uid' => $cc_uid));
				if ($crs['rst']) {
			...
			$where = array('c_id1'=>$old_c_id1, 'c_id2'=>$old_c_id2, 'cc_uid'=>$cc_uid);
...
//            	$this->_log('修改名人堂用户：data=' . serialize($data) . ', where=' . serialize($where), 'update');
	...
	function delStar()
	{
		...
		if (empty($c_id1) || empty($c_id2) || empty($cc_uid)) {
			$this->_error('参数错误！', URL('mgr/celeb_mgr.starList'));
		}
		
		$rs = DR('Celeb.delStarUser', '', $c_id1, $c_id2, $cc_uid);
		if ($rs['rst']) {
//			$this->_log('删除名人堂用户：c_id1=' . $c_id1 . ', c_id2=' . $c_id2 . ', cc_uid=' . $cc_uid, 'delete');
