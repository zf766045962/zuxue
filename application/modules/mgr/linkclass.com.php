...
{ 
	$db = APP :: ADP('db');
	$table=$db->getTable(T_LINK_CLASS);
...
//*********************添加分类保存到数据库操作***********************
	function save_class($data,$lmid=''){
		...
		$sql="insert into ".$table."(classid,classname,classurl,parentid,parentpath,depth,rootid,child,previd,nextid,orderid,readme,elite,ontop,Author,keyword,description,lmorder,uunique,pictureurl,statue) values('".$classid."','".$classname."','".$classurl."',".intval($parentid).",'".$parentpath."',";
		...
		if($re){
		echo "<script language=javascript>alert('添加成功！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
		exit;
		}else{
...
//echo $sql;exit;
	...
	function delclass($id){
		...
		$ru=$db->execute("delete From ".$table." where classid = " . $classid);
...
//*****************保存修改分类到数据库***************************
	function modify_class($data){
		...
		$sql="update ".$table." set classname='".$classname."',readme='".$readme."',lmorder='".$lmorder."',elite='".$elite."',ontop='".$ontop."',keyword='".$keyword."',description='".$description."',uunique='".$uunique."',pictureurl='".$pictureurl."',classurl='".$classurl."' where classid='".$classid."'";
		$re=mysql_query($sql);

		if($re){
		echo "<script language=javascript>alert('修改成功！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
		exit;
		}else{
		echo "<script language=javascript>alert('操作失败，请重试！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
		exit;
		}
	...
	{
		$founderr=true;
		echo "<script language=javascript>alert('参数不足，请重试！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
		exit;
	}
	...
	{
		$founderr=true;
		echo "<script language=javascript>alert('找不到指定栏目！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
		exit;
	}
	...
	if($rs_all["parentid"]!=$rparentid)
	{
		...
		{
			$founderr=true;
			echo "<script language=javascript>alert('所属栏目不能为自己！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
			exit;
		}
		...
		if(intval($rs_all["parentid"])==0)
		{
			if($rparentid>0)
			{
				...
				{
					$founderr=true;
					echo "<script language=javascript>alert('不能指定外部栏目为所属栏目！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
					exit;
				}
				else
				{
					...
					{
						$founderr=true;
						echo "<script language=javascript>alert('不能指定该栏目的下属栏目作为所属栏目！');location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
						exit;
					}
				}
			}
  ...
  echo "<script>location='".URL('mgr/linkclass.classlist&lmid='.$lmid)."';</script>";
	...
	function xietui($id){
	   ...
	   $a=$db->execute($sql);
	   if($a){
		   echo "<script>alert('成功解除推荐');history.back(-1);</script>";
		   exit;
	   }else{
		   echo "<script>alert('操作失败，请重试！');history.back(-1);</script>";
		   exit;
	...
	function tuijian($id){
	   ...
	   $a=$db->execute($sql);
	   if($a){
		   echo "<script>alert('成功设为推荐');history.back(-1);</script>";
		   exit;
	   }else{
		   echo "<script>alert('操作失败，请重试！');history.back(-1);</script>";
		   exit;
	...
	function xieding($id){
	   ...
	   $a=$db->execute($sql);
	   if($a){
		   echo "<script>alert('成功解除置顶');history.back(-1);</script>";
		   exit;
	   }else{
		   echo "<script>alert('操作失败，请重试！');history.back(-1);</script>";
		   exit;
	...
	function zhiding($id){
	   ...
	   $a=$db->execute($sql);
	   if($a){
		   echo "<script>alert('成功设为置顶');history.back(-1);</script>";
		   exit;
	   }else{
		   echo "<script>alert('操作失败，请重试！');history.back(-1);</script>";
		   exit;
