...
class link
{
	...
	{
		$db = APP :: ADP('db');
		$sql = "select * from ".$db->getTable(T_LINKCLASS)." where bid=".$bid." order by lmorder asc";
		$data = $db->query($sql);
		return RST($data);
	}
	...
	{
		$db 		= APP :: ADP('db');
		$sql 		= "select id,bid,classid,title,times,recommend,audit,top,lmorder from ".$db->getTable(T_LINK)." where id>0 ";
		$sql_count 	= "select count(id) as count from ".$db->getTable(T_LINK)." where id>0 ";
		$sqlstr 	= "";
		if(!empty($bid))
		{
			$sqlstr .= " and bid=".$bid." ";
	...
	{
		$db = APP :: ADP('db');
		$sql = "update ".$db->getTable(T_LINK)." set ".$flagtype."=".$flagvalue." where id in (".$id.")";
		$rs = $db->query($sql);
		return RST($rs);
	}
	//删除信息
	...
	{
		$db = APP :: ADP('db');
		$sql = "delete from ".$db->getTable(T_LINK)." where id in (".$id.")";
		$rs = $db->query($sql);
		return RST($rs);
	}
	...
	{
		$db = APP :: ADP('db');
		$sql = "select classname,classid from ".$db->getTable(T_LINKCLASS)." ";
		if(intval($classid)>0)
		{
	...
	{
		$db = APP :: ADP('db');
		$sql = "select * from ".$db->getTable(T_LINKCLASS)." where classid=".$id."";
		$data = $db->getRow($sql);
		return RST($data);
	}
	//保存分类信息
