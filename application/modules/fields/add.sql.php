...
switch($field_type) {
	...
	case 'smallint':
		$minnumber = intval($minnumber);
		$defaultvalue = intval($defaultvalue);
		$this->db->query("ALTER TABLE `$tablename` ADD `$field` SMALLINT ".($minnumber >= 0 ? 'UNSIGNED' : '')." NOT NULL DEFAULT '$defaultvalue'");
	break;
	...
	case 'mediumtext':
		$this->db->query("ALTER TABLE `$tablename` ADD `$field` MEDIUMTEXT NULL");
	break;
	
	case 'text':
		$this->db->query("ALTER TABLE `$tablename` ADD `$field` TEXT NULL");
	break;
	...
	case 'timestamp':
		$this->db->query("ALTER TABLE `$tablename` ADD `$field` TIMESTAMP NULL");
	break;
	...
	case 'title':
		...
		$sql = "ALTER TABLE `$tablename` ADD `$field` VARCHAR( $maxlength ) NOT NULL DEFAULT '$defaultvalue'";
		$this->db->query($sql);
		$this->db->query("ALTER TABLE `$tablename` ADD `style` VARCHAR( 30 ) NULL");
	break;
	case 'islink':
		if(!$maxlength) $maxlength = 255;
		$maxlength = min($maxlength, 255);
		$this->db->query("ALTER TABLE `$tablename` ADD `islink` INT( 1 ) NOT NULL DEFAULT '0'");
		$this->db->query("ALTER TABLE `$tablename` ADD `$field` VARCHAR( $maxlength ) NULL");
	break;
	case 'relatedata':
		if($_POST['setting']['showtype'] == 1){
			$this->db->query("ALTER TABLE `$tablename` ADD `$field` VARCHAR( $maxlength ) NULL");
			$this->db->query("ALTER TABLE `$tablename` ADD `{$field}_idstr` INT( 8 ) NULL");
		}else{
			$this->db->query("ALTER TABLE `$tablename` ADD `$field` MEDIUMTEXT NULL");
			$this->db->query("ALTER TABLE `$tablename` ADD `{$field}_idstr` TEXT NULL");
		}
	break;
	/*case 'pages':
