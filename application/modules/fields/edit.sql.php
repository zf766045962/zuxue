...
switch($field_type) {
	case 'varchar':
		...
		$sql = "ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` VARCHAR( $maxlength ) NOT NULL DEFAULT '$defaultvalue'";
		$this->db->query($sql);
	break;
	...
	case 'mediumtext':
		$this->db->query("ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` MEDIUMTEXT NULL");
	break;
	
	case 'text':
		$this->db->query("ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` TEXT NULL");
	break;
	...
	case 'timestamp':
		$this->db->query("ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` TIMESTAMP NULL");
	break;
	...
	case 'islink':
		if(!$maxlength) $maxlength = 255;
		$maxlength = min($maxlength, 255);
		$sql = "ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` VARCHAR( $maxlength ) NULL";
		$this->db->query($sql);
	break;
	case 'relatedata':
		if($_POST['setting']['showtype'] == 1){
			$this->db->query("ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` VARCHAR( $maxlength ) NULL");
			$this->db->query("ALTER TABLE `$tablename` CHANGE `{$oldfield}_idstr` `{$field}_idstr` INT( 8 ) NULL");
		}else{
			$this->db->query("ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` MEDIUMTEXT NULL");
			$this->db->query("ALTER TABLE `$tablename` CHANGE `{$oldfield}_idstr` `{$field}_idstr` TEXT NULL");
		}
	break;
	/*case 'pages':
	break;
