...
<table cellpadding="2" cellspacing="1" width="98%">
	...
	<tr> 
	  <td>是否作为区间字段</td>
	  <td>
	  <label><input type="radio" name="setting[rangetype]" value="1" <?php if($setting['rangetype']) echo 'checked';?> /> 是 </label>
	  <label><input type="radio" name="setting[rangetype]" value="0" <?php if(!$setting['rangetype']) echo 'checked';?> /> 否</label> 　　注：区间字段可以通过filters('字段名称','模型id','自定义数组')调用
	  </td>
	</tr>		
</table>