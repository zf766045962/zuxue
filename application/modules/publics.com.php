...
class publics{
	...
	function date1($table,$set,$where){ 
		$db = APP :: ADP('db'); 
		$sql = "update xsmart_".$table." set " .$set. " where " .$where; 
		//echo $sql; 
		$data = $db->query($sql); 
		return RST($data); 
	}
...
//echo $sql;
	...
	*/
	...
	function date2($table,$line,$where){
		   $db = APP :: ADP('db');  
		   $sql = "insert into xsmart_$table ($line) values ($where)";  
		   //echo $sql;//die();
		   $data = $db->query($sql);
     	   return RST($data);
		}

function page_list33($pagesize=1,$where='',$order,$getData,$table,$item){
		extract($getData);
		$pagesize 	= empty($pagesize) ? 1 : $pagesize;
		$total 		= DS('publics.get_total33','',$table,$where);
		if(isset($page)){
			$offset = ($page-1) * $pagesize;
		}else{
			$offset = 0;
		...
		$limit = "$offset,$pagesize";
		$result['info'] = DS('publics.get_info','',$table,$where,$order,$limit,$item);
		unset($getData["page"]);
		$param 	= http_build_query($getData);
		$url 	= "?".$param."&page={page}";
	...
	function get_total33($table,$where = ''){
		$db = APP :: ADP('db');
		$sql = "select count(distinct(tid)) as num from ".$db->getTable($table);
		if($where != ''){
			$sql .= " where ".$where;
		}
		//echo $sql;
