...
class pager
{
	...
	function makePage($vName='page'){
		...
		*/
		...
		$return = "<span><A href=".$this->_creRouteUrl(1)."> 首页 </a></span><span>".$return."</span><span><A href=".$this->_creRouteUrl($pageCount)."> 尾页 </a></span>";
		$return = "共计".$recordCount."条记录&nbsp;".$return;
		$return = str_replace("总记录数 ".$recordCount,'',$return);
		$return .=" 页次：".$currentPage."/".$pageCount."页 ".$this->getParam('pageSize')."条/页 ";
		$return .='<select onchange="window.location=this.value">';
		for($i=1;$i<=$pageCount;$i++)
		{
			...
			$return .='<option value="'.$this->_creRouteUrl($i).'" '.$flag.'>第'.$i.'页</option>';
		}
		$return .='</select>';
		//var_dump($return);
		return $return;
