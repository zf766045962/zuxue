...
$GLOBALS['___xwbStData'] = array(
'POST_INPUT' => 'php://input',
'OAuthRequest_version'=>'1.0a',
'boundary'=>''
);

function ___throwEception($str){
...
class OAuthSignatureMethod_RSA_SHA1 extends OAuthSignatureMethod {
    ...
    function fetch_public_cert(&$request) {
        ...
        ___throwEception("fetch_public_cert not implemented");
    }
    ...
    function fetch_private_cert(&$request) {
        ...
        //
        // either way should return a string representation of the certificate
        ___throwEception("fetch_private_cert not implemented");
    }
...
class OAuthRequest {
        ...
        if (!$parameters) {
            ...
            if ($http_method == "POST"
                ...
                $post_data = OAuthUtil::parse_parameters(
                    file_get_contents($GLOBALS['___xwbStData']['POST_INPUT'])
                );
    ...
    function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters=NULL) {
        @$parameters or $parameters = array();
        $defaults = array("oauth_version" => $GLOBALS['___xwbStData']['OAuthRequest_version'],
    ...
    function to_header() {
        ...
        foreach ($this->parameters as $k => $v) {
            ...
            if (is_array($v)) {
                ___throwEception('Arrays not supported in headers');
            }
            $out .= ',' .
...
class OAuthServer {
    ...
    function get_version(&$request) {
        ...
        if ($version && $version != $this->version) {
            ___throwEception("OAuth version '$version' not supported");
        }
        return $version;
    }
    ...
    function get_consumer(&$request) {
        ...
        if (!$consumer_key) {
            ___throwEception("Invalid consumer key");
        }

        $consumer = $this->data_store->lookup_consumer($consumer_key);
        if (!$consumer) {
            ___throwEception("Invalid consumer");
        }
    ...
    function get_token(&$request, $consumer, $token_type="access") {
        ...
        if (!$token) {
            ___throwEception("Invalid $token_type token: $token_field");
        }
        return $token;
    }
    ...
    function check_signature(&$request, $consumer, $token) {
        ...
        if (!$valid_sig) {
            ___throwEception("Invalid signature");
        }
    }
    ...
    function check_nonce($consumer, $token, $nonce, $timestamp) {
        ...
        if ($found) {
            ___throwEception("Nonce already used: $nonce");
        }
    }
...
class OAuthUtil {
    ...
    function build_http_query_multi($params) {
        ...
        $GLOBALS['___xwbStData']['boundary'] = $boundary = uniqid('------------------');
		$MPboundary = '--'.$boundary;
