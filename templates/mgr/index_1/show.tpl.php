...
<? $ssa = str_replace('&','>',$ssa)?>
        ...
        <form action="<?php echo URL('mgr/index_1.show','&classid='.V('r:classid'))?>" name="form1" method="post" id="form1" onsubmit="return check()">
		...
		<select name="select">
					 ...
					 <option <?= V('r:select')=='dateline'?'selected="true"':''?> value= "dateline"  >发帖时间</option> 
					 <option <?= V('r:select')=='looknum'?'selected="true"':''?> value= "looknum"  >阅读数量</option> 
					 <option <?= V('r:select')=='alltip'?'selected="true"':''?> value= "alltip"  >热度</option> 		
				    
		</select>	
		
		 <span style="display:inline-block; line-height:24px;">板块：</span>
		<select name="fidd">
					 <option value= "" >所有板块</option> 	
					 <? $date = DS('publics._get','','bbs_forum','status = 1');?>
						<? foreach($date as $k=>$v){?>
						<option <?= V('r:fidd')==$v["fid"]?'selected="true"':''?> value="<?=$v['fid']?>"><?=$v['name']?></option>
	    			<? }?>		
				    
		</select>
		
		 <span style="display:inline-block; line-height:24px;">状态：</span>
		<select name="showindex">
					 <option value= "" >全部状态</option> 	
					 <option <?= V('r:showindex')=='1'?'selected="true"':''?> value= "1" >已显示</option> 
					 <option <?= V('r:showindex')=='0'?'selected="true"':''?> value= "0" >未显示</option>  
		</select>
			 ...
			 $couunm1= DS('publics.get_total','','bbs_postcomment',"rpid = 0 and comment != '' ".' and tid='.$val['pid'] . ' and dateline'.'>'. $beginYesterday.' and dateline'.'<'. $endYesterday);
				...
				<div id="myModal<?=$val['pid']?>" class="reveal-modal">	
					...
					<tr onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#EEF7FF'">
							...
							<td><a href="javascript:;" data-reveal-id="myModal<?=$val['pid']?>"><?=htmlspecialchars($val["subject"])?></a></td>
							<td><?=$val["author"]?></td>
							...
							<td>
							<a class="" href="javascript:is_showindex(<?=$val['pid']?>);"><?=$val["is_showindex"]==1?"取消显示":"<em style='color:red'>显示</em>"?></a>&nbsp;
							<?php /*?><a class="" href="javascript:;" onclick="alerts(<?=$val['pid']?>)" data-reveal-id="myModalshow"><?=$val["showindex"]==0?"设置热":"<em style='color:red'>取消热</em>"?></a>	<?php */?>
		...
		function is_showindex(id){
			var xmlhttp;
					if (window.XMLHttpRequest){
						...
						xmlhttp.open("GET","<?=URL("mgr/index_1.is_showindex",'&id=')?>"+id,true);
						xmlhttp.send();		
			} 
