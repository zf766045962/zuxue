	...
	$(function(){
		...
		$('#BackupBnt').click(function(){
			...
			Box.confirm('提示','确认要备份吗？',function(id){
				if(id == 'ok'){
					...
					Req.postReq('<?php echo URL('mgr/backup.backupData') ?>',{},function(e){
						if(e.isOk()){
							location.reload();
						} else {
		...
		$('a[name="restore"]').click(function(){
			...
			Box.confirm('提示','确认还原 '+$(this).attr('rel')+' 的备份吗？',function(id){
				if(id == "ok"){
					Req.postReq('<?php echo URL('mgr/backup.restore') ?>',{name:sthis.attr('rel')},function(e){
						if(e.isOk()){
							//top.location.href = Req.mkUrl('mgr/admin','logout');
							Box.success('提示','恭喜您，操作成功，数据已经还原');
						} else {
							Box.alert('提示',e.getMsg());
						}
						bLock =1;
		...
		$('#removeAll').click(function(){
			...
			Box.confirm('提示','确认要执行批量删除操作吗',function(id){
				if(id == "ok"){
					Req.postReq('<?php echo URL('mgr/backup.remove') ?>',{flags:flags.join(',')},function(e){
						if(e.isOk()){
							location.reload();
						} else {
							Box.alert('提示',e.getMsg());
...
<body class="main-body">
    ...
    <div class="main-cont">
    	...
    	<div class="set-area">
			<div class="form-s1">
				<ul>
					<li>1.数据备份会备份你的Framework所有的数据库数据，你可以在通过数据恢复还原备份的数据</li>
		...
		<div class="set-area">
			<p class="tips">本功能在恢复备份数据的同时，将全部覆盖原有数据，且在还原期间会导致不能正常访问。</p>
		</div>
