...
<body class="main-body">
	<div class="path">
	  <p>当前位置：后台管理<span>&gt;</span><?=$adname?>分类管理<span>&gt;</span><?=$adname?></p></div>
        <div class="main-cont">
        <h3 class="title"><a class="btn-general" href="<?php echo URL('mgr/ad.addclass&bid='.$bid)?>"><span>添加<?=$adname?>分类</span></a>
        <a class="btn-general"  href="<?php echo URL('mgr/ad.classlist&bid='.$bid)?>"><span><?=$adname?>分类列表</span></a>
        <?= $adname?>分类列表</h3>
		<div class="set-area">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
				...
				<tr onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#EEF7FF'">
                   ...
                   <td><div class="td-nowrap"><a href="<?=URL("mgr/ad.addclass","bid=".$rs["bid"]."&classid=".$rs["classid"]."&id=".$rs["classid"]."")?>"><?php echo $rs['classname']?></a></div>
                   <td><div class="td-nowrap"><?php echo $rs['uunique']?></div>
                   </td>
                    <td><a class="icon-edit" href="<?=URL("mgr/ad","bid=".$rs["bid"]."&classid=".$rs["classid"]."")?>">编辑内容</a></td>
                    <td><?=$rs["lmorder"]?></td>
                    <td>
                    	 <a class="icon-add" href="<?=URL("mgr/ad.addclass","bid=".$rs["bid"]."&classid=".$rs["classid"]."")?>">添加子栏目</a>
                       <a class="icon-edit" href="<?=URL("mgr/ad.addclass","bid=".$rs["bid"]."&classid=".$rs["classid"]."&id=".$rs["classid"]."")?>">修改</a>
                        <a class="icon-del" href="javascript:delConfirm('<?=URL("mgr/ad.delclass","bid=".$rs["bid"]."&id=".$rs["classid"]."")?>','您确定要删除此分类信息吗？');">删除</a>
                    </td>
                    </tr>
                    <?php
							}
                   ...
                   <td><div class="td-nowrap">┋┣&nbsp;<?php echo $rss['uunique']?></div>
                    ...
                    <td><a class="icon-edit" href="<?=URL("mgr/ad","bid=".$rss["bid"]."&classid=".$rss["classid"]."")?>">编辑内容</a></td>
                    <td><?=$rss["lmorder"]?></td>
                    <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                       <a class="icon-edit" href="<?=URL("mgr/ad.addclass","bid=".$rss["bid"]."&classid=".$rss["classid"]."&id=".$rss["classid"]."")?>">修改</a>
                        <a class="icon-del" href="javascript:delConfirm('<?=URL("mgr/ad.delclass","bid=".$rss["bid"]."&id=".$rss["classid"]."")?>','您确定要删除此分类信息吗？');">删除</a>
                    </td>
                    </tr>
