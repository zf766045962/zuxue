<div class="form-box">	
    <form name="config" id="form1" action="<?php echo URL('mgr/page_manager.doEditComponent', array('page_id'=>$page_id));?>" method="post">
		...
		<?php if (!in_array($data['component_id'], array(16, 21))) {?>
        <div class="form-row">
            <label class="form-field">标题</label>
            <div class="form-cont">
                <input class="input-txt w130" type="text" vrel="_f|ne=m:不能为空" warntip="#titleErr" name="data[title]" value="<?php echo V('r:title', F('escape', $data['title'])); ?>"/>
        ...
        <div class="form-row" >
			...
			<div class="form-cont">
                <select name="param[list_id]" id="listIdSelect"   rel="sel5">
                    ...
                    <?php 
                        DR('Xsmart/xwb.setToken', FALSE, 2);	// Set The Site Tocken
                        $rs   		= DR('Xsmart/xwb.getUserLists', FALSE, SYSTEM_CC_UID);
                        ...
                        DR('Xsmart/xwb.setToken', FALSE, 1);	// Set The Normal Tocken
                    ?>
                </select>
            
            	<input type="hidden" name="param[uid]" value="<?php echo SYSTEM_CC_UID;?>">
        ...
        <?php
             ...
             $list 		= DR('Xsmart/topics.getCategoryByType');
             $topic_get	= isset($data['param']['topic_get']) ? (int)$data['param']['topic_get'] : 0;
        ...
        <div class="form-row">
                ...
                <div class="form-cont">
                    ...
                    <div id="topic_idLabel" <?php if($topic_get > 0) {echo " style='display:none;' ";} ?>>
                        ...
                        <?php if (isset($data['param']['topics']) && is_array($data['param']['topics'])) {foreach ($data['param']['topics'] as $index => $topic) {?>
                        <p class="input-item">
							<?php if (isset($data['param']['tid'][$index]) && $data['param']['tid'][$index] != '') {?>
							<input type="text" class="input-txt w130 input-disabled" name="param[topics][]" value="<?php echo $topic?>" readonly /> 
							<?php } else {?>
							<input type="text" class="input-txt w130" name="param[topics][]" value="<?php echo $topic?>" /> 
							<?php }?>
							<a class="icon-del" href='javascript:;' onclick="delIcs(this);">删除</a>
							<input type="hidden" value="<?php echo isset($data['param']['tid'][$index])? $data['param']['tid'][$index]: '';?>" name="param[tid][]">
							</p>
                        <?php }}?>
                        <p class="input-item"><input type="text" class="input-txt w130" name="param[topics][]" /> 
							<a class="icon-add" href="javascript:;" onclick="addIcs(this)">添加</a>
							<a href="javascript:;" class="icon-add" onclick="addSpeIcs(this);">添加专题</a>
						</p>
<!--
...
-->								
			...
			<div  class="hidden">
				<select id="speIcsList">
					<?php
					$pages = DR('PageModule.getPagelistByType','', 1);
		...
		<?php } elseif (in_array($id, array(20, 22))) { // 专题介绍?> 
		<div class="form-row">
			<label class="form-field">文字介绍<br />(支持HTML)</label>
            <div class="form-cont">
				<textarea class="input-area w250" name="param[desc]"><?php echo isset($data['param']['desc']) ? htmlspecialchars($data['param']['desc']) : ''; ?></textarea>
            </div>
		</div>
		<?php } elseif (21 == $id) { // 专题介绍?> 
		<div class="form-row">
			<label class="form-field">话题</label>
            <div class="form-cont">
				<input name="param[topic]" value="<?php echo isset($data['param']['topic']) ? $data['param']['topic'] : ''; ?>" />
            </div>
		</div>
		<?php } elseif (23 == $id) { // 视频?> 
		<div class="form-row">
			<label class="form-field">视频代码</label>
            <div class="form-cont">
				<textarea class="input-area w250" name="param[code]"><?php echo isset($data['param']['code']) ? htmlspecialchars($data['param']['code']) : '输入视频播放HTML代码'; ?></textarea>
            </div>
            <div class="form-cont">
				提示：请注意自行修改视频播放HTML代码的宽高，最大为180px宽度*162px高度。 
            </div>
		</div>
        <?php }  
