...
<body class="main-body">
	<div class="path">
	  <p>当前位置：后台管理<span>&gt;</span><?= $linkname;?>分类管理<span>&gt;</span></p></div>
        <div class="main-cont">
        <h3 class="title">
		<a class="btn-general" href="<?= URL('mgr/eat.classlist&bid='.$bid)?>"><span><?= $linkname;?>分类列表</span></a>
		<?= isset($info['classid']) && intval($info['classid']) > 0 ? "修改" : "添加";?><?=$linkname;?>分类 
        </h3>
        <div class="set-area">
        <form  name="form1a" id="form1a" method="post" action="<?= URL('mgr/eat.saveclass')?>">
                <div class="form web-info-form">
                    ...
                    <div class="form-cont">
                        <?= $editor->image(1,'imgurl',isset($info) && !empty($info) ? $info["imgurl"] : '','上传图片','class="input-txt"');?>
                    </div>
                    </div>
                      
                    <div class="form-row"><label class="form-field">图片预览</label>
                    <div class="form-cont">
                        <img src="<?= isset($info) && !empty($info) ? $info["imgurl"] : '';?>" id="imgurlView"/>
                    </div>
                    </div>-->
                      
                    <!--<div class="form-row"><label class="form-field">栏目链接地址</label>
                    <div class="form-cont">
                    <input name="classurl" id="classurl" class="input-txt" type="text" size="50"  value="<?=isset($info) && !empty($info) ? $info["classurl"] : "";?>"></div>
                    </div>-->
                    
                    <!--<div class="form-row"><label class="form-field">介绍</label> 
                    <div class="form-cont">
                    <textarea name="readme" id="readme" class="input-area area-s4 code-area" cols="10" rows="10"><?= isset($info) && !empty($info) ? $info["readme"] : "";?></textarea></div>
                    </div>-->
                    ...
                    <!--<div class="form-row"><label class="form-field">旅游指南</label>
                    <div class="form-cont">
                    <textarea name="description" id="description"><?= isset($info) && !empty($info) ? $info["description"] : "";?></textarea>
                    </div>
                    ...
                    <div class="btn-area">
                  	 		<a class="btn-general highlight" name="<?= isset($info['classid']) && intval($info['classid']) > 0 ? "确认修改" : "确认添加"; ?>" onclick="$('#submitBtn').click();"><span><?= isset($info['classid']) && intval($info['classid']) > 0 ? "确认修改" : "确认添加" ;?></span></a>
