...
<body class="main-body">
	<div class="path">
	  <p>当前位置：后台管理<span>&gt;</span><?=$casesname?>分类管理<span>&gt;</span><?=$casesname?></p></div>
        <div class="main-cont">
        <h3 class="title"><a class="btn-general" href="<?php echo URL('mgr/cases.addclass&bid='.$bid)?>"><span>添加<?=$casesname?>分类</span></a>
        <a class="btn-general"  href="<?php echo URL('mgr/cases.classlist&bid='.$bid)?>"><span><?=$casesname?>分类列表</span></a>
        <?=$casesname?>分类列表</h3>
		<div class="set-area">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table">
				...
				<tr onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#EEF7FF'">
                   ...
                   <td><div class="td-nowrap"><?php echo $rs['uunique']?></div>
                    ...
                    <td><a class="icon-edit" href="<?=URL("mgr/cases","bid=".$rs["bid"]."&classid=".$rs["classid"]."")?>">信息列表</a></td>
                    <td><?=$rs["lmorder"]?></td>
                    <td>
                    	 <a class="icon-add" href="<?=URL("mgr/cases.addclass","bid=".$rs["bid"]."&classid=".$rs["classid"]."")?>">添加子栏目</a>
                       <a class="icon-edit" href="<?=URL("mgr/cases.addclass","bid=".$rs["bid"]."&classid=".$rs["classid"]."&id=".$rs["classid"]."")?>">修改</a>
                        <a class="icon-del" href="javascript:delConfirm('<?=URL("mgr/cases.delclass","bid=".$rs["bid"]."&id=".$rs["classid"]."")?>','您确定要删除此分类信息吗？');">删除</a>
                    </td>
                    </tr>
                    <?php
							}
                   ...
                   <td><div class="td-nowrap">┋┣&nbsp;<?php echo $rss['uunique']?></div>
                    ...
                    <td><a class="icon-edit" href="<?=URL("mgr/cases","bid=".$rss["bid"]."&classid=".$rss["classid"]."")?>">信息列表</a></td>
                    <td><?=$rss["lmorder"]?></td>
                    <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                       <a class="icon-edit" href="<?=URL("mgr/cases.addclass","bid=".$rss["bid"]."&classid=".$rss["classid"]."&id=".$rss["classid"]."")?>">修改</a>
                        <a class="icon-del" href="javascript:delConfirm('<?=URL("mgr/cases.delclass","bid=".$rss["bid"]."&id=".$rss["classid"]."")?>','您确定要删除此分类信息吗？');">删除</a>
                    </td>
                    </tr>
