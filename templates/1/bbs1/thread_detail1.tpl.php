...
<link href="http://bbs.meizu.cn/thread-5285291-1-1.html" rel="canonical" />
<meta name="keywords" content="阿里巴巴,发布会" />
<meta name="description" content=" 10 月 21 日魅族阿里巴巴举行战略合作发布会 " />
...
<script type="text/javascript">
	var STYLEID = '1', STATICURL = 'static/', IMGDIR = 'http://127.0.0.1:8004/images', VERHASH = 'Y2F', charset = 'utf-8', discuz_uid = '9594205', cookiepre = 'MZBBS_2132_', cookiedomain = '', cookiepath = '/', showusercard = '0', attackevasive = '0', disallowfloat = 'login|newthread|tradeorder|activity|debate|usergroups|task', creditnotice = '', defaultstyle = '', REPORTURL = 'aHR0cDovL2Jicy5tZWl6dS5jbi90aHJlYWQtNTI4NTI5MS0xLTEuaHRtbA==', SITEURL = 'http://127.0.0.1:8004/', JSPATH = 'http://127.0.0.1:8004/js/';  
// 是否是手机浏览器
...
<style id="diy_style" type="text/css"></style>
	...
	<div class="wp cl">
		...
		<input type="text" class="p_fre z" id="p_fre" size="4" onkeyup="value=value.replace(/[^1234567890-]+/g,'');ssnn()"  onkeydown="if(event.keyCode==13 && this.value !== '' ) {window.location=$('fj_btn').href;return false;}" title="跳转到指定楼层" /></span></h3>
<!--兼容删除的帖子直接跳过-->

<script>
	function ssnn(){
		$('fj_btn').href=' <?=URL('bbs1.lctz','&tid='.$tid.'&all='.$all2[0]['alltip'].'&lctz')?>'+document.getElementById('p_fre').value
	}
</script>
...
<? }?>
    ...
    width: 54px" id="quoteDelBtn1"><img src="images/cl.png" ></div></div>
				...
				<div id='yc1' style='display:none'>
					<textarea rows="6" cols="80" id='content2' value='' onmousedown="s(event,this)"> 
					 
					</textarea>
				</div>
				<div id='yc'>
					<textarea rows="6" cols="80" id='content3' value=''  onmousedown="s(event,this)"> 
...
您需要登录后才能回复&nbsp;&nbsp;&nbsp;<a class="xi2" href="<?= URL('bbsUser.login')?>">登录</a><a class="separator"></a><a class="xi2" href="<?= URL('bbsUser.register')?>">立刻注册</a>
		...
		<?php */?>
			...
			var ccImg = document.getElementById("checkCodeImg"); 
				...
				var ms =document.getElementById("load11").innerText;
				...
				if(ms1.length == 0){
					...
					alert('验证码不正确')		
						...
						var chk = document.getElementById('fastpostrefresh');
							if(chk.checked){
								location.href='index.php?m=bbs1.connment'+'&names='+con3+'&tid='+<?=$tid?>+'&fid='+<?=V('r:fid')?>+'&page='+<?=$num?>;
							}else{
								location.href='index.php?m=bbs1.connment'+'&names='+con3+'&tid='+<?=$tid?>+'&fid='+<?=V('r:fid')?>; 
							}				
			...
			function aa(){	
				...
				var zhfid =document.getElementById("zhf").innerText;
				...
				if(ms1.length == 0){
					...
					alert('验证码不正确')		
						...
						location.href='index.php?m=bbs1.zhfsql'+'&names='+zhfid+'&con3='+content2+'&tid='+<?=$tid?>+'&page='+<?=V('r:page') !=NULL?V('r:page'):1?>+'&fid='+<?=V('r:fid')?>; 
					}else{
						alert('您没输入任何内容')
						}	
...
<script type="text/javascript" reload="1">updateseccode5('SsDkD4b0','follow_rebroadcast');</script>
				...
				<p class="ptm pnpost cl">
					...
					<em id='sbhl'> 
					<label for="fastpostrefresh" class="wrap_simcheck" style="float:left;margin:0px;">
