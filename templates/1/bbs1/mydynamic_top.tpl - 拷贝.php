<div id="flw_header" class="mbw bbs cl" style=" background-color:#FFFFFF; border:#e6e6e6 1px solid;">
    <div class="follow_avatar" style="position:relative;" id="follow_avatar" onmouseover="ssbb()">
        <a class="avatar" href="<?= URL('bbsUser.my_avatar',"&uid=$uid")?>">
            <?php
				//echo $_SESSION['u_uid'];
				$re1 = DS('publics._get','','users',"id='".$uid."'");
				// var_dump($re1);
				
				//判断用户是否设置头像
				if(empty($re1[0]['head_img'])){
            ?>
            	<img src="images/w50h50.gif"  onerror="this.onerror=null;this.src='images/noavatar_big.gif'" />
            <?php
				}else{
			?>
            	<img src="<?php echo($re1[0]['head_img'])?>" />	
            <?php
				}
			?>	
        ...
        <div class="follow_avatar_absolute" style="display:none;" id="follow_avatar_absolute">
        	<a href="<?= URL('bbsUser.my_avatar',"&uid=$uid")?>">修改头像</a>
        </div>                       
    </div>
    <div class="broadcast" style="width:578px;_width:577px; border-left:#e6e6e6 1px solid;" >
    	<div class="broadcast_top" >
            <div class="uname_broadcast" title="用户<?= $re1[0]['username']?>">
                <span class="inner_uname">用户<?= $re1[0]['username']?></span>
                <span class="cl"></span>
            </div>
        	<div style="float:right; padding-top:5px;">
                <span class="c">广播</span><span class="f"><a href="<?= URL('#')?>"><?= $re1[0]['newpm']?></a></span>
                <span class="c">收听</span><span class="f"><a href="<?= URL('#')?>"><?= $re1[0]['newpm']?></a></span>
                <span class="c">听众</span><span class="f"><a href="<?= URL('#')?>"><?= $re1[0]['newpm']?></a></span>
            </div>
        	<div class="cr"></div>
    	</div>
...
<script type="text/javascript">
	...
	<form method="post" id="fastpostform" action="<?= URL('')?>">
                ...
                <div  id="fastposteditor">
                    <div class="tedt2" style="background-color:#f9f9f9; _width:576px;_overflow:hidden;">
                        <div class="area" style="background-color:#f9f9f9;" >
                        	<textarea style="height:55px;width:554px;font-size:14px; padding: 5px 0 0 20px;outline:none;" cols="70" name="message" id="fastpostmessage" value='你在做什么呢？' onKeyDown="seditor_ctlent(event, '$(\'fastpostsubmit\').click()');" tabindex="12" class="pt2 xs2"></textarea>
                        </div>
                        <div id="flw_bar" class="bar2" style="_width:569px;height:39px;*height:39px;" >
                            <div style="float:left; width:120px; margin-top:8px;">
                                <div class="fpd"><a href="javascript:;" class="fsml" title="表情" id="fastpostsml" onclick="showMenu({'ctrlid':this.id,'evt':'click','layer':2});return false;"><em></em></a>
...
<script type="text/javascript" reload="1"> wysiwyg = undefined;</script>
                            ...
                            <div style="float:right; margin:0px; padding:0px; width:435px;">
                                <button name="topicsubmit_btn" id="fastpostsubmit" value="topicsubmitbtn" tabindex="13" class="pn pnc" style=" float:right;" >发表</button>
...
<script src="js/bbsjs/fileprogress.js" type="text/javascript"></script>
            ...
            <!--<div id="flw_post_subject" style="display:none;_width:596px;" >
                <span id="flw_checklen" class="float_number" ><span id="checklen" class="xg1">70</span></span>
