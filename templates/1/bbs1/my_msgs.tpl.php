				...
				<dl notice="6652481" class="cl" onmouseover="showin7(<?=$v['id']?>)">
													<table cellspacing="0" cellpadding="0">
														...
														<col width="744px">
															...
															<th>
																<div class="avt mbn">
<a class="avatar" href="<?=URL('bbsUser.user_broadcast','&id='.$v['uid'])?>"><img src="<?=$user[0]['head_img']==NULL?"images/noavatar_big.gif":$user[0]['head_img']?>" style="cursor: pointer;"><span class="shadowbox_avatar"> </span></a>
<? if($v['is_show'] == 0){?>
...
<?	}?>
	...
	<div class="ntc_body readntc_body">
																												<a href="<?=URL('bbsUser.user_broadcast','&id='.$v['uid'])?>"><?=$user[0]['username']?></a> 发给您的信息 <a target="_blank" href="<?=URL('bbsUser.my_msgs','&ccid=1')?>" class="closenotice"><?=htmlspecialchars($v['information'])?></a> &nbsp; <a class="lit" target="_blank" href="<?=URL('bbs2.space','&uid='.$v['uid'].'&followuid='.$_SESSION['u_uidss'])?>">查看 ›</a>                                            </div>
...
<?	}?>
															...
															<td>
																											<div class="ntc_body readntc_body">
																												<a href="<?=URL('bbsUser.user_broadcast','&id='.$v['uid'])?>"><?=$user[0]['username']?></a> 回复了您的帖子 <a target="_blank" href="<?=URL('bbs.thread_detail','&tid='.$v['tid'].'&fid='.$v['fid'])?>" class="closenotice"><?=htmlspecialchars($v['information'])?></a> &nbsp; <a class="lit" target="_blank" href="<?= URL('bbs2.lczda','&lcid='.$tidid[0]['id'].'&tid='.$v['tid'].'&fid='.$v['fid'])?>">查看 ›</a>                                            </div>
																<div class="time_msgnotice">
<span class="xg1 xw0"><span title="<?=date("Y-m-d H-i-s",$v['addtime'])?>"><?=date("m月d号",$v['addtime'])?></span></span>
					...
					<dl notice="6641167" class="cl" onmouseover="showin7(<?=$v['id']?>)">
													<table cellspacing="0" cellpadding="0">
														...
														<col width="744px">
															...
															<th>
																<div class="avt mbn">
	<a class="avatar"><img alt="systempm" src="<?=$user[0]['head_img']==NULL?"images/noavatar_big.gif":$user[0]['head_img']?>"><span class="shadowbox_avatar"> </span></a>
	<? if($v['is_show'] == 0){?>
	<img style="width:12px;height:12px;margin:-5px 0 0px 34px" src="images/pm_unread22.png">
<?	}?>
</div>
		...
		<a href="<?=URL('bbsUser.user_broadcast','&id='.$v['uid'])?>"><?=$user[0]['username']?></a> 收听了您。<a href="<?=URL('bbsUser.my_follow','&ccid=2')?>">点击查看 ›</a>      
		                                      </div>
																<div class="time_msgnotice">
																	<span class="xg1 xw0"><span title="<?=date("Y-m-d H-i-s",$v['addtime'])?>"><?=date("m月d号",$v['addtime'])?></span></span>
																	<a title="删除" id="a_note_<?=$v['id']?>" href="javascript:;"  onclick="delxingx(<?=$v['id']?>)" class="shield_msgnotice">删除</a>
					...
					<dl notice="5543691" class="cl" onmouseover="showin7(<?=$v['id']?>)">
                            	<table cellspacing="0" cellpadding="0">
                                    ...
                                    <col width="744px">
                                    	...
                                    	<th>
                                            <div class="avt mbn">
             <a class="avatar"><img alt="systempm" src="images/logo22.png"><span class="shadowbox_avatar"> </span></a>
			 <? if($v['is_show'] == 0){?>
	<img style="width:12px;height:12px;margin:-5px 0 0px 34px" src="images/pm_unread22.png">
<?	}?>
                                                                                            </div>
			...
			<div class="pgs cl pagebar">
				...
				<? $information = DS('publics._get','','information',"id not in ($newstrss) and status = 1 and ".'addtime'. '>'. $useridaddtime[0]['addtime'] ." order by  addtime desc");?>
