...
<? $id = V('r:id')==NULL?$_SESSION['u_uidss']:V('r:id') ?>
    ...
    <div class="page_frame_navigation" >
        ...
        <ul class="mbw tb cl page_frame_ul" style="padding-left:20px;" >
             <li class="a"  ><a href="<?= URL('bbsUser.my_dynamic','&bottom=1')?>">关注</a></li>
            <li ><a href="<?= URL('bbsUser.my_dynamic','&bottom=2')?>">大厅</a></li>
    ...
    <div class="flw_feed">
        ...
        <?php
			...
			$all4=DS('publics.get_total','','bbs_postcomment', "authorid in ($newstrss) and comment != '' and  mood != 1 or poststatus = 1 and authorid in ($newstrss)"); 
			$re2 = DS('publics._get','','bbs_postcomment',"authorid in ($newstrss) and comment != '' and  mood != 1 or poststatus = 1 and authorid in ($newstrss) ORDER BY dateline desc limit 0,10");	
					
					
					//$re2 = DS('publics.get_info','','bbs_postcomment',"comment != '' and authorid='".$av['followuid']."' and mood != 1",'dateline desc');
			...
			<li class="cl" name = "lili" id="feed_li_<?= $pv['authorid']?>" onmouseover="this.className='flw_feed_hover cl'" onmouseout="this.className='cl'">
            	<div class="feed_li_box" >
                	<div class="z flw_avt">
						...
						?>
                        <img src=<?= $re4[0]['head_img']?> isdrift="true"><span class="shadowbox_avatar"> </span></a>
                        <?php
							}else{
						?>
                        <img src='images/w100h100.jpg' isdrift="true"/><span class="shadowbox_avatar"> </span></a>
                        <?php
							} 
						?>
    					<span class="cnr"></span>
					...
					<div class="flw_article" style=" " >
                		<div class="flw_author">
                            ...
                            <a class="name_feedlist" href="<?= URL('bbsUser.user_broadcast',"&id=".$pv['authorid']."")?>"><?= $re4[0]['username']?>&nbsp;&nbsp;</a> 发表于 <span title="<?= date('Y-m-d H:i:s',$pv['dateline'])?>">
								<?php 
						...
						<? if($pv['classid'] == 1){?>
			<div id="original_content_15038108" class="pbm c cl"><?  echo htmlspecialchars($pv['comment'])?></div>
							<? }else{ ?>
				
			<a href=" <?= URL('bbs2.lczda','&lcid='.$pv['id'].'&tid='.$pv['tid'].'&fid='.$pv['pid'])?>" target="_blank" > 
	...
	<script>
		...
		function insert(id,tid){
			...
			if (window.XMLHttpRequest){
				...
				xmlhttp.open("GET","<?=URL('bbs2.insers'),'&con='?>"+encodeURIComponent(con)+'&tid='+encodeURIComponent(tid)+'&fid='+pid+'&id='+id+'&loid='+encodeURIComponent(loid),true);
				xmlhttp.send();	
			}else{
				alert('请输入内容')
				}	
...
<ul class="list_replybox" id="newreply_5313176_12918932">
	...
	<?php /*?>	<? }else if($pv['classid'] == 1){?>
		
				<?	 $name = DS('publics._get','','bbs_postcomment',"comment != '' and tid=".$pv['tid'] .' order by dateline desc limit 0,6');?>
				<? foreach($name as $vv22 ){?> 
				<li><a class="d xi2" href="<?= URL('bbsUser.user_broadcast',"&id=".$vv22['authorid']."")?>"><?=$vv22['author']?>&nbsp;&nbsp;</a><?=$vv22['comment']?></li>
				<? }?>	
		<? }?><?php */?>
...
<script type="text/javascript">
	spaceClosedFun();
            ...
            <div style=" padding:20px 0px;" class="emp">
			<h2 style="margin:30px 0;" class="mbw xg1 xs2 hm">还没有关注的内容</h2>
        </div>
            <?php
				}
			?>
	...
	<?php ?><? if($pk == 9){?>
		<div class="flw_more" id="loadingfeed"><a class="xi2" onclick="loadmore4(<?=$id?>);return false;" href="javascript:;"  id = "moreless">更多 »</a></div>
	<? } ?><?php ?>
		...
		//alert(num);
				...
				var xmlhttp;
					if (window.XMLHttpRequest){
						...
						xmlhttp.open("GET","<?=URL("bbs2.lorderlist2",'&newlist='.$newstrss.'&num=')?>"+num+'&id='+id,true);
						xmlhttp.send();	
