					...
					<div class="thmenu">
                        <div class="page_frame_navigation" >
                            ...
                            <ul class="mbw tb cl page_frame_ul" style="padding-left:20px;" >
                                <li  class="a"  ><a href="<?= URL('bbsUser.my_submit',"&cid=1")?>">主题</a></li>
...
<table class="conbar_guide" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<div class="select_box select_box_1" style="float:left; position:relative;">
			<div class="box_menu" value="" vl="" style="color:#1daeed;" filter="true" id="box_menu">
            <?php 
				if(empty($status)){
			?>	
            	全部
			...
			<div class="son_menu" style="position:absolute; display:none;width:118px;" id="son_menu">
				<ul>
					<li value="0" vl="0"  class="one" onclick="selStatus(this.value)">全部</li>
                    <li value="1" vl="common" onclick="selStatus(this.value)"  class="" >已发表</li>
                    <li value="2" vl="save"   onclick="selStatus(this.value)"  class="" >草稿</li>
                    <li value="3" vl="close"  onclick="selStatus(this.value)"  class="" >关闭</li>
                    <li value="4" vl="aduit"  onclick="selStatus(this.value)"  class="" >待审核</li>
				</ul>
			</div>
		</div><?php */?>
        <script>
		...
		<div class="select_box select_box_2" style="float:left; position:relative;">
			...
			<div class="son_menu" style="position:absolute; display:none;width:138px;height:320px;overflow-y:scroll;" id="son_menu1">
				<ul>
					...
					<? $plantlist = DS('publics._get','','bbs_forum','');?>
    	...
    	<td class="by">板块</td><td class="by">作者</td><td class="num">回复/查看</td><td class="by">发表时间</td><td class="by">操作</td>
	</tr>
</tbody>
</table>
	</div>
    <script>
			...
			<script>
				function selForum(forumId){
					//alert(forumId)
					window.location.href = "<?= URL('bbsUser.my_submit',"&forum_id=")?>"+forumId;
				}
			</script>
...
<table cellspacing="0" cellpadding="0"  class="guide_theme" >	
	<?php
		...
		{
			foreach($re as $pk => $pv){
		    $plant = DS('publics._get','','bbs_forum','fid='.$pv['fid']);	
	?>			
	
	<tbody id="normalthread_5297412">
		<tr>
			<th class="common"><div class="title_guidelist" style="width:248px"><a href="<?= URL('bbs.thread_detail','&fid='.$pv['fid']."&tid=".$pv['pid'])?>" target="_blank" class="xst" title="<?= $pv['subject']?>"><?= htmlspecialchars($pv['subject'])?></a></div></th>
			<td class="by"><a href="<?= URL('bbs.thread','&fid='.$pv['fid'])?>" target="_blank"><?= $plant[0]['name']?></a></td>
			<td class="by"><cite><a href="<?= URL('bbsUser.user_broadcast','&id='.$pv['authorid'])?>" c="1"><?= $pv['author']?></a></cite></td>
			<td class="num"><a href="<?= URL('bbs.thread_detail','&fid='.$pv['fid'].'&tid='.$pv['pid'])?>" class="xi2" target="_blank"><?= $pv['alltip']?></a><div class="br"><em><?= $pv['looknum']?></em></div></td>
			<td class="by" style="padding-right:20px;"><div class="br"><em><?php /*?><a href=""><?php */?><span title="<?= date("m月d日",$pv['dateline'])?>"><?= date("m月d日",$pv['dateline'])?></span></a></em></div></td>
            <td class="by"><a href="javascript:;" onclick="delpost(<?= $pv['pid']?>)">删除</a>/<a href="<?= URL('bbs.thread','&fid='.$pv['fid'])?>" target="_blank">前往</td>
            </tr>
	</tbody>
    <?php
			}
