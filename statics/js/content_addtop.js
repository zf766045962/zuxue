...
function dialog(url,title,width,height,id){
	var id = arguments[4] ? arguments[4] : 'dialog';
	return $.dialog({
		title:title,
		id: id,
		width: width,
...
function change_images(urlList,returnid){
	if(urlList == '') return true;
	$('#'+returnid+'_tips').css('display','none');
	var str = $('#'+returnid+'picList').html();
	//var contents = in_content.split('|');
	//var filenames = in_filename.split('|');
	$.each(urlList, function(i, data) {
		var ids = parseInt(Math.random() * 10000 + 10*i);
		var filename = data.name;
		str += "<li id='image"+ids+"'><input type='text' name='"+returnid+"_url[]' value='"+data.url+"' style='width:310px;' ondblclick='image_priview(this.value);' class='input-text' readonly> <input type='text' name='"+returnid+"_alt[]' value='"+filename+"' style='width:160px;' class='input-text' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\"> <input type='text' name='"+returnid+"_sort[]' value='0' style='width:50px;' class='input-text'> <a href=\"javascript:remove_div('image"+ids+"')\">移除</a> </li>";
	});
	$('#'+returnid+'picList').html(str);
}
...
function image_priview(img) {
	...
	//$.dialog({title:'图片查看（5秒后自动关闭）',fixed:true, content:'<img src="'+img+'" />',id:'image_priview',time:5});
}
