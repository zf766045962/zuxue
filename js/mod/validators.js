...
.reg('f', function(elem, data){
    ...
    if(chain.context.comForm && (tn == 'input' || tn == 'textarea')){//设置焦点样式
    	...
    	var fe = chain.context.errCss;
    	var okTip = chain.context.okTip;
	    $(elem).blur(function(){
			focusCssName=$(this).attr(fc);
			//alert(errCssName);
	    	$(this).removeClass(focusCssName);
	    	if(!data.ch)//失去焦点检测
	        	chain.validate();
	    }).focus(function(){
			errCssName=$(this).attr(fe);
	    	$(this).removeClass(errCssName);
			focusCssName=$(this).attr(fc);
	    	$(this).addClass(focusCssName);
        	okTipId = $(elem).attr(okTip);
			if(okTipId){
				if(this.haveSpace) {
					$(okTipId).css('visibility', 'hidden');
				} else {
					$(okTipId).addClass('hidden');
				}
			}
...
.reg('url', function(elem, v, data, next) {
    if(v){
    	var result = /[a-z]/.test(v);
        if(!data.m && data.m !== 0)
            data.m = getText('请输入正确的域名格式');
    	this.report(result, data);
    }else this.report(true, data);
	next();	
...
.reg('chinese', function(elem, v, data, next) {
    if(v){
    	var result = /^[\u4e00-\u9fa5],{0,}/.test(v);
        if(!data.m && data.m !== 0)
            data.m = getText('请输入汉字');
    	this.report(result, data);
    }else this.report(true, data);
	next();	
...
.reg('phone', function(elem, v, data, next) {
    if(v){
    	var result = /((\d{3}-\d{8})|(\d{4}-\d{7})|(^1\d{10}))$/.test(v);
        if(!data.m && data.m !== 0)
            data.m = getText('请输入正确的电话号码');
    	this.report(result, data);
    }else this.report(true, data);
	next();	
...
.reg('english', function(elem, v, data, next) {
    if(v){
    	var result = /^[a-zA-Z0-9_-]+$/.test(v);
        if(!data.m && data.m !== 0)
            data.m = getText('请输入正确的字符串');
    	this.report(result, data);
    }else this.report(true, data);
	next();	
...
.reg('IDCard', function(elem, v, data, next) {
    if(v){
    	var result = /[\d]{6}(19|20)*[\d]{2}((0[1-9])|(11|12))([012][\d]|(30|31))[\d]{3}[xX\d]$/.test(v);
        if(!data.m && data.m !== 0)
            data.m = getText('请输入正确的身份证号码');
    	this.report(result, data);
    }else this.report(true, data);
	next();	
