...
//
    ...
    onerror : function(elem, data, chain){
		...
		//liu  聚焦时css
		var errCssName = $(elem).attr(this.errCss);
        if(errCssName){
			$(elem).addClass(errCssName);
		}
    ...
    tipPass : function(elem){
        var tipId = $(elem).attr(this.errTipName);
    	var okTipId = $(elem).attr(this.okTip);
		...
		if(okTipId){
			//todo  @@liu
           $(okTipId).cssDisplay(true);
			//$(okTipId).show();
			//cssDisplay(true);
		}
		
		if(okTipId && 1==0){
			$(okTipId).className=('tips-error');
			alert($(okTipId).className);
			if(this.haveSpace) {
           	 //$(okTipId).cssDisplay(false);
				$(okTipId).className=('tips-error');
