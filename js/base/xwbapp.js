...
/**
 * @class Xwb
 * 本类是所有Xsmart JavaScript交互应用的命名空间根目录。
...
var Util = X.util = {
        ...
        * @param {Boolean} [cascade] cascade apply template from value
        ...
        <pre>
            <code>
                var name = templ('My name is {name}', {name:'Xsmart'});
        ...
        */
         ...
         * 返回对象查询字符串表示形式.
         * <pre><code>
           var obj = {name:'Xsmart', age:'25'};

           //显示 name=Xsmart&age=25
 ...
 * <p>HTML模板字符解析依赖两个数据：
   ...
   <pre>
    ...
    alert( parse('Box', map) );
    结果是：&lt;h2&gt;Xsmart&lt;/h2&gt;&lt;div&gt;名称是Xsmart&lt;/div&gt;
    </pre>
  * @singleton
  */
 ...
 */
    upload : function( callback ){
        if(this.beforeUpload(this.form) !== false){
            ...
            this.formEl.action = Util.appendParam(this.action, {callback:'parent.'+fn, '_':Util.uniqueId(),verifyhash:X.getCfg('verifyhash')});
            this.formEl.submit();
        }
    },
