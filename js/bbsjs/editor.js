...
var editorcurrentheight = 322, editorminheight = 322, savedataInterval = 30, editbox = null, editwin = null, editdoc = null, editcss = null, savedatat = null, savedatac = 0, autosave = 1, framemObj = null, cursor = -1, stack = [], initialized = false, postSubmited = false, editorcontroltop = false, editorcontrolwidth = false, editorcontrolheight = false, editorisfull = 0, fulloldheight = 0, savesimplodemode = null;
EXTRAFUNC['keydown'] = [];
EXTRAFUNC['keyup'] = [];
EXTRAFUNC['mouseup'] = [];
...
function newEditor(mode, initialtext) {
	...
	if(editorMode == 'simple'){//根据cookie中存储的用户的选择，切换编辑器高级或简单模式
		$('e_modeswitch').style.display = 'none';
		$('e_simplemode').style.display = 'none';
		$('e_advancemode').style.display = 'block';
		if($('post_extra')){//如果存在更多操作选项栏
			$('post_extra').style.display = 'none';
		}
	}else{
		$('e_modeswitch').style.display = 'block';
		$('e_advancemode').style.display = 'none';
		$('e_simplemode').style.display = 'block';
		if($('post_extra')){
			$('post_extra').style.display = 'block';
		}
	}
...
function initEditor() {
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		...
		$(editorid + '_fullswitcher').className = 'xg1 ed2';
	}
...
function pasteWord(str) {
	...
	if(mstest.test(str)){
		...
		str = str.replace(/<(\w[^>]*) style="([^"]*)"([^>]*)/gi, function ($1, $2, $3, $4) {
			...
			re = new RegExp('(^|[;\\s])margin-left:\\s*([^;]+);?', 'ig');
			match = re.exec($3);
			if(match != null){
				style += 'margin-left:' + parseInt(parseInt(match[2]) / 10) + 'em;';
			}
			if(style) {
				style = ' style="' + style + '"';
			}
...
function checkFocus() {
	if(wysiwyg) {
		try {
			if(navigator.appName == 'Netscape'){//兼容Netscape搜狗高速模式
	...
	if(wysiwyg) {
		...
		if(initialized && !(BROWSER.firefox && BROWSER.firefox >= '3' || BROWSER.opera)) {
			...
			text = '<!DOCTYPE html PUBLIC "-/' + '/W3C/' + '/DTD XHTML 1.0 Transitional/' + '/EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">' +
				...
				'</head><body style="background-color:#FFFFFF;">' + text + '</body></html>';
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'attach', 'image', 'pasteword']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		showDialog('您确认要清除所有内容吗？', 'confirm', '清除内容',function(){clearContent();});
		...
		editorform.target = "";
	} else if(cmd == 'wordprocess'){
		var style = $('wordprocess_box').style.display;
		if(style == 'block'){
			$('wordprocess_box').style.display = 'none';
		}else{
			$('wordprocess_box').style.display = 'block';
		}
	} else if(cmd == 'more'){
		$('more_box').style.display = 'block';
		$('e_more').style.display = 'none';
	} else if(cmd == 'simplemode'){//若用户选择简单模式，切换编辑器到简单模式，并存储到cookie中
		$('e_modeswitch').style.display = 'none';
		$('post_extra').style.display = 'none';
		$('e_simplemode').style.display = 'none';
		$('e_advancemode').style.display = 'block';
		setcookie('editorMode','simple',2592000);
	} else if(cmd == 'advancemode'){//若用户选择简单模式，切换编辑器到简单模式，并存储到cookie中
		$('e_modeswitch').style.display = 'block';
		$('post_extra').style.display = 'block';
		$('e_advancemode').style.display = 'none';
		$('e_simplemode').style.display = 'block';
		setcookie('editorMode','advance',2592000);
	} else {
	...
	if(in_array(cmd, ['bold', 'italic', 'underline', 'strikethrough', 'inserthorizontalrule', 'fontname', 'fontsize', 'forecolor', 'backcolor', 'justifyleft','justifycenter', 'justifycright', 'insertorderedlist', 'insertunorderedlist', 'floatleft', 'floatright', 'removeformat', 'unlink', 'undo', 'redo'])) {
		hideMenu();
	}
	doane();
	return ret;
...
// 隐藏所有弹出菜单
function menus_hide_all(){
	menusobj	= $j("#e_menus").find(".fwinmask");
	for(i=0;i<menusobj.length;i++){
		id	= $j(menusobj[i]).attr('id');
		if(id.indexOf('_menu')!=-1 ){
			$j(menusobj[i]).css('visibility', 'hidden');
		}
	}
}
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = '请输入链接地址<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 96%;margin-bottom:10px;" value="" class="px" />'+
					(selection ? '' : '<br />请输入链接文字<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 96%;margin-bottom:10px;" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'free':
				...
				str += lang[tag] + '<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" style="display:none;" />只有用户回复本帖时显示</label><br /><div style="display:none;" ><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />只有当浏览者积分高于</label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs px3" /> 时才显示</div><br /><label>有效天数设置:</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs px3" /> <br />大于这个天数时将自动失效<div style="height:10px;"></div>' : '');
				break;
			case 'tbl':
				str = '<p class="pbn">表格行数 <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px px3"/> &nbsp; 表格列数 <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px px3" /></p><p class="pbn">表格宽度 <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px px3" /><span style="display:none;"> &nbsp; 背景颜色 <input type="text" id="' + ctrlid + '_param_4" size="2" class="px px3" onclick="showColorBox(this.id, 2)" /></span></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'小提示\', null, 0)"><img id="tbltips" title="小提示" class="vm" src="' + IMGDIR + '/info_small.gif"> 快速书写表格提示</p>';
			...
			case 'aud':
				...
				break;
			case 'vid':
				str = '<p class="pbn">请输入视频地址</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" class="px" /></p><p class="pbn">宽 <input id="' + ctrlid + '_param_2" size="5" value="500" class="px px2" /> &nbsp; 高 <input id="' + ctrlid + '_param_3" size="5" value="375" class="px px2" /></p><p class="xg2 pbn">支持优酷、土豆、56、酷6等视频站的视频网址<br />支持 wmv avi rmvb mov swf flv 等视频格式<br />示例 http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">请输入 Flash 文件地址</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">宽 <input id="' + ctrlid + '_param_2" size="5" value="" class="px px2"/> &nbsp; 高 <input id="' + ctrlid + '_param_3" size="5" value="" class="px px2" /></p><p class="xg2 pbn">支持 swf flv 等 Flash 网址<br />示例 http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = '从 Word 粘贴内容';
		...
		if(menupos == '00') {
			menu.className = 'fwinmask';
			s = '<table width="100%" cellpadding="0" cellspacing="0" class="fwin"><tr><td class="m_c">'
				...
				+ '<p class="o pns"><button type="submit" id="' + ctrlid + '_submit" class="normalbtn bluebtn"><strong>提交</strong></button></p>'
				+ '</td></tr></table>';
		} else {
			s = '<div class="p_opt cl"><span class="closebtn_at"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">关闭</a></span><div class="tcontent" >' + str + '</div><div class="pns mtn btnbar_opt"><button type="submit" id="' + ctrlid + '_submit" class="normalbtn bluebtn"><strong>提交</strong></button></div></div>';
		}
...
function switchEditor(mode) {
	...
	setCaretAtEnd();
	try{
		checkFun_items_click['.wrap_full_screen']	= false;
	}catch(e){}
}
