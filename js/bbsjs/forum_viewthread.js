...
function showauthor(ctrlObj, menuid) {
	authort = setTimeout(function () {
		...
		var url = ctrlObj.getAttribute("data-href");
		if($(menuid + '_ma').innerHTML == '') $(menuid + '_ma').innerHTML = '<a href='+url+' target="_blank">'+ctrlObj.innerHTML+'</a>';
	}, 500);
...
function fastpostappendreply() {
	if($('adddynamic') != null){
		setcookie('adddynamic', $('adddynamic').checked ? 1 : 2, 2592000);
	}
	...
	div.className = 'viewthread_table item_postlist graybar_postlist';
	$('postlistreply').appendChild(div);
	//$('postlistreply').className = 'viewthread_table item_postlist graybar_postlist';
...
function succeedhandle_fastpost(locationhref, message, param) {
	...
	removeAllChild("fastposthiddenview");
	ifmDelQuoteFunc();//发表后清空表单隐藏域
}
...
function appendreply() {
	newpos = fetchOffset($('post_new'));
	document.documentElement.scrollTop = newpos['top'];
	document.body.scrollTop = newpos['top'];//兼容chrome
	var newClass = $('post_new').className;
	$('post_new').style.display = '';
	$('post_new').id = '';
	div = document.createElement('div');
	div.id = 'post_new';
