...
function bbcode2html(str) {
	...
	if(!allowhtml || !fetchCheckbox('htmlon')) {
		...
		if(!fetchCheckbox('parseurloff')) {
			str_quote	= str.indexOf('[/quote]');
			if(str_quote>0){
				str_quote_prefix	= str.substr(0,str_quote);
				str_quote_postfix	= str.substr(str_quote);
				str_quote_postfix 	= parseurl(str_quote_postfix , 'html', false);
				str 	= str_quote_prefix+str_quote_postfix;
			}else{
	...
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
		...
		str = str.replace(/\[size=(\d+?)\]/ig, '<font size="$1" class="fontsize_$1">');
...
function dstag(options, text, tagname) {
	...
	if(in_array(tagname, ['div', 'p'])) {
		...
		align_2 = getoptionvalue('style', options);
		if(in_array(align, ['left', 'center', 'right'])) {
			prepend = '[align=' + align + ']' + prepend;
			append += '[/align]';
		}else if(in_array(align_2, ['text-align: left;', 'text-align: center;', 'text-align: right;'])){//居左、居中、居右内容结尾处添加换行
			text += '\n';
		}else{
			if(BROWSER.chrome){//chrome浏览器所见即所得模式下enter换行切换到文本模式换行消失问题
				prepend = '\n' + prepend;
			}else{
				append += '\n';
			}
...
function html2bbcode(str) {
	...
	if(!fetchCheckbox('parseurloff')) {
		str_quote	= str.indexOf('</blockquote>');
		if(str_quote>0){
			str_quote_prefix	= str.substr(0,str_quote);
			str_quote_postfix	= str.substr(str_quote);
			str_quote_postfix 	= parseurl(str_quote_postfix , 'bbcode', false);
			str 	= str_quote_prefix+str_quote_postfix;
		}else{
		...
		str=str.replace(r,"<a$1>$2$3</a>");
		
		var r=/<a(.*?)>(.*?)\[\/url\](.*?)<\/a>/;
		str=str.replace(r,"<a$1>$2$3</a>");
		str=str.replace("<brcv/>","");
