...
function validatesavedraft(){
        if(postSubmited == true){return false;}
	var message = wysiwyg ? html2bbcode(getEditorContents()) : $j('#message').val();
	if($j('#subject').val() == "" || trim(message)== "") {
		showError('抱歉，您尚未输入标题或内容');
		return false;
	}else if( mb_strlen($j('#subject').val() ) > 80){
                showError('您的标题超过 80 个字符的限制');
		return false;
        }else if($j("#typeid").val() == '0'){
                showError('请选择主题对应的分类');
		return false;
        }else{
            $j('#postsave').attr('value',1);
            $('postsubmit').click();
        }
       //return true;
}
function validate(theform) {
	...
	/*
	// 这里不要了。。
	//message 	= "[url=forum.php?mod=redirect&goto=findpost&pid=107270896&ptid=4721604][quote][size=2][color=#999999]songyaping[/color] [img]static/image/common/mzvip3.jpg[/img] [/size]wwwderewrewww.baidu.com[/quote][/url][url=http://www.baidu.com]www.baidu.com[/url]";
	//message 	= "[url=http://www.baidu.com]（呆因明中）[url]www.baidu.com[/url][url=http://www.baidu.com]（呆因明中）[url]www.baidu.com[/url][/url]"
	if(!theform.parseurloff.checked) {
		message = parseurl(message);
		
		// 去掉 URL 标签嵌套A标签
		// [url=http://www.baidu.com]（呆因明中）[url]www.baidu.com[/url][/url]
		var r=/\[url(.*?)\](.*?)\[url\](.*?)\[\/url\]/;
		message	= message.replace(/\r\n/,"<brcv/>");
		message	= message.replace(r,"[url$1]$2$3[/url]");
		
		var r=/\[url(.*?)\](.*?)\[\/url\](.*?)\[\/url\]/;
		message	= message.replace(r, "[url$1]$2$3[/url]" );	//"[url$1]$2$3[/url]"
		message	= message.replace("<brcv/>","");
	...
	*/
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		var msg = '内容太少或者太多了，请控制在' + postminchars + '~' + postmaxchars + '字节之间';
		tipPop("#postsubmit",msg,5000);
		return false;
	}
	if(UPLOADSTATUS == 0) {
...
function uploadNextAttach() {
	...
	if(arr[4] == 'ban') {
		...
		sizelimit = '(不能超过 ' + arr[5]/1024 + ' kb)';
	} else if(arr[4] > 0) {
		sizelimit = '(不能超过 ' + arr[4]/1024 + ' kb)';
	}
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		if(arr[7] == 'ban') {
			...
			sizelimit = '(不能超过 ' + arr[8]/1024 + ' kb)';
		} else if(arr[7] > 0) {
			sizelimit = '(不能超过 ' + arr[7]/1024 + ' kb)';
		}
...
function uploadWindow(recall, type) {
	...
	showWindow('upload', 'forum.php?mod=misc&action=upload&fid=' + fid + '&type=' + type, 'get', 0, {'zindex':951});
}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			...
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc normalbtn bluebtn"><strong>使用</strong></button></p>', 'info', '未使用的' + (type == 'attach' ? '附件' : '图片'));
	} else if(op == 3) {
