...
function switchAdvanceMode(url) {
	...
	if(obj.reppost.value != ''){//如果是带引用的回复
		url = url + '&repquote=' + obj.reppost.value;//修改URL
	}
	var msg = $("fastpostiframe").contentWindow.document.body.innerHTML;
	obj.message.value = msg.slice(0,-7);//给隐藏的textarea赋值
...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = '内容太少或者太多了，请控制在' + postminchars + '~' + postmaxchars + '字节之间';
	}
	if(s) {
		showError(s);
		doane();
...
function loadData(quiet, formobj) {
	...
	showDialog('此操作将覆盖当前帖子内容，确定要恢复数据吗？', 'confirm', '恢复数据',function(){  recover_data(formobj);});
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			html    = '<span id="forumnewshow"><a class="notice_newforum" href="javascript:void(0);"><em>有新回复的主题，点击查看</em><em class="noticeicon_newform"></em></a></span>'
...
function addtbodyrow2(table, insertID, changename, separatorid, jsonval) {
	...
	o 			= '';
	var column = document.getElementById('separatorlineItem').getAttribute('colspan');//判断当前列表页table列数
	try{
		if(thread['o'] && column == 4){//当前列表是4列的情况下才会加入复选框列，为避免更换用户组后更新列表时错乱问题。
