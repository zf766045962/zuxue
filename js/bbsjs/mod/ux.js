...
mod.CommentArea = X.reg('CmtArea', Util.create(Base, {
    ...
    decorateLoading : function(){
        ...
        }else $(this.getView().parentNode).find('#Xsmart_loading').remove();
...
//	cache:[],
	...
	reload : function(key){
		...
		if( rst !== undefined ) { 
			...
			this.oldXhr = Req.q(Req.apiUrl('action','getAtUsers'),{ keyword:this.key ,verifyhash : getCfg('verifyhash') },Util.getBind(this,'loaded'));
		}
	},
	//根据关键字搜索或存储缓存
 ...
 * @singleton
 */
X.reg('myMsg', Util.create(ui.Box,{
    ...
    checkText : function(){
		var len = getCfg('hasDirectMessages') ? 140 : 300;
        var v = $.trim(this.jqContent.val()),
            left = Util.calWbText(v, len);
        this.jqWord.html(
            left >= 0 ? getText('您还可以输入{0}字', this.warp(left) ) : getText('已超出{0}字', this.warp( Math.abs(left) ))
        );
        return left>=0 && v;
    },
	...
	warp:function(i){
		if(!this.warpNode){
			...
			return '<'+this.warpNode+'>' + i +'</'+this.warpNode+'>';
		}
	},
    
    afterShow : function(){
        ui.Base.prototype.afterShow.call(this);
    ...
    onSendLoad : function( e ){
        if(e.isOk()){
           !this.dialogMode && this.display(false);
		   this.dialogCallBack && this.dialogCallBack(e);
        }else {
            var msg;
            switch(e.getCode()){
                case 20014:
