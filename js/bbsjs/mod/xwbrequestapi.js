 ...
 */
    act : function(name, data, success, cfg){
		...
		if(data){
			data['verifyhash'] = X.getCfg('verifyhash');
		} else {
			data={'verifyhash':X.getCfg('verifyhash')};
		}
 ...
 */
    ...
    saveLocalpmSet : function(data, fn, cfg){
        XwbRequest.act('saveLocalpmSet', data, fn, cfg);
    },
/**
 * 发送提醒。
...
/**
 * 	增加Note @@liu
 */ updateNote : function(data, fn, cfg){
	 	alert('asd');
        XwbRequest.act('saveNote', data, fn, cfg);
    },
