	...
	$(function(){
		...
		$( "div.drag-area div" ).disableSelection();
		...
		//修改页面标题信息
		$('#doEditPageData').click(function(){
			var newPageName = encodeURIComponent($('#new_page_name').val());
			Xwb.request.postReq( mkUrl('mgr/page_manager','doEditPageData'),
								'page_id='+pageID+'&page_name='+ newPageName,
								function(r){
									if(r.isOk()){
										Xwb.ui.MsgBox.success('提示','保存成功');
									} else {
										Xwb.ui.MsgBox.alert('提示',r.getMsg());
									}
								}
		...
		$('#setBackground').click(function(){
			Xwb.use('Dlg',{
				cs:'win-fixed',
				dlgContentHtml:'uploadBackGroup',
				closeable:true,
				mask:true,
				appendTo:$('body'),
				buttons:[
				...
				onactiontrig:function(e){
					if(e.get('e') == 'bgPlace'){
						this.w = e.get('w');
						this.jq('#align a').removeClass('cur');
						$(e.src).addClass('cur');
					}
				},
				onViewReady:function(){
					...
					this.jq('#xwb_back_file').change(function(){self.change(this.value)});
					
					this.jq('.skin-set').css({
						borderBottomWidth:0
					});
					this.jq('.skin-setbg').css({
						borderRightWidth:0,
						width:390
					});
					this.jq().css('marginLeft',-220);
					this.jq('#pid').val(pageID);
		    	...
		    	,getUploader : function(){
			        if(!this.uploader)
			            this.uploader = Xwb.use('Uploader', {
			                form:this.jqForm, 
			                action :mkUrl('mgr/page_manager','uploadBackground'),
			                onload:Util.getBind(this, 'onUploadLoad')
			            });
			            
			        return this.uploader;
		    	},
				onUploadLoad:function(r){
					if(r.isOk()){
						this.uploadFile = r.getData();
						this.jq('#previewImg').attr('src',this.uploadFile);
					} else {
				...
				onbuttonclick:function(id){
					if(id == 'ok'){
						var obj = {
							align:this.w,
							bg:this.uploadFile,
							repeat:+this.jq('#bg-repeat')[0].checked,
							fixed:+this.jq('#bg-fixed')[0].checked,
							pid:pageID
						},self = this;
						if(!obj.bg){
							MsgBox.alert('提示','请上传图片');
							return;
						}
						Xwb.request.postReq(mkUrl('mgr/page_manager', 'setBackground'),obj,function(r){
							if(r.isOk()){
								self.close()
							} else {
								MsgBox.alert('提示',r.getMsg());
				...
				afterDisplay:function(){
					...
					switch (+component_id){
						...
						case 11:
							...
							break;
						case 21 : 
							var input = popWin.dlg.jq("input[name='param[topic]']");
								input.focus();
							break;
					}
		...
		function addIcs(sthis){
		    var $inp = $(sthis).prev('input');
			if($inp.val() !='' ){
			    $(Xwb.ax.Tpl.parse("topicItem",{value : $inp.val()})).insertBefore($(sthis).parent());
				$inp.val('');
		...
		var _popSelBox;
		function addSpeIcs(sthis) {
		    console.log("endter")
		    var $list = $("#speIcsList");
			if(!_popSelBox){
			    console.log("first");
		    	_popSelBox =  Xwb.use('Box', {
  					cs:' win-moudle-topic hidden',
  					appendTo: "body",
    				closeable : true,
					autoRender : true,
					contextable : true,
					onViewReady:function(v){
					    var self = this;
  						$(self.view).find("#xwb_dlg_ct").append($list).append('<p id="xwb_err_tip" style="color:red" class="hidden">该项已添加</p>');
						
						$list.change(function(e){
							var items = $("#topic_idLabel p.input-item input[name='param[tid][]']"),
							    tid = $list.val();
							for(var i=0, l=items.length; i < l; i++){
								if(items[i].value == tid){  
									$(self.view).find("#xwb_err_tip").removeClass("hidden");
									return ;
								}
							}
							$(self.view).find("#xwb_err_tip").addClass("hidden");
							if(tid == "" || tid == "0")
								return ;
						
							var $lastItem = $("#topic_idLabel p.input-item:last"),  
							topicName = $list.find("option")[$list[0].selectedIndex].innerHTML;

							$(Xwb.ax.Tpl.parse("topicItem",{ 'value' : topicName, 'cls' : 'input-disabled', 'tid' : tid, 'readonly' : 'readonly' ,
								hd_input : '<input type="hidden" value="'+tid+'" name="param[tid][]">'
							})).insertBefore($lastItem);
						
							self.display(isShow = false);
                        });
					},
    				showAt : function(anchor){
					    $list[0].selectedIndex = 0;   //清空选项
						$(this.view).find("#xwb_err_tip").addClass("hidden");
			...
			$(_popSelBox.view).hasClass("hidden") ?  _popSelBox.showAt(sthis) : _popSelBox.display(false);
		}
