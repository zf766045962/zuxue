...
(function() {
	friendSelector = function(parameter) {
		...
		this.handleWpObj = $(parameter['searchWpId']);
		...
		this.imgBtn = null;
		this.initialize();
		this._uname = parameter['searchId'];
	...
	friendSelector.prototype = {
		...
		selectUserName:function(userName) {
			...
			if(userName != '') {
				...
				if(uid && typeof this.selectUser[uid] == 'undefined' || uid === 0 && typeof this.selectUser[userName] == 'undefined') {
					...
					spanObj.className = "unamecard_msg";
					if(uid) {
					...
					this.selectNumber++;
					spanObj.innerHTML= '<em class="z" title="' + userName + '">' + userName + '</em><a href="javascript:;" class="x" onclick="'+this.handleKey+'.delSelUser(\''+(spanObj.id)+'\');">删除</a><input type="hidden" name="users[]" value="'+userName+'" uid="uid'+uid+'" />';
					this.handleWpObj.parentNode.insertBefore(spanObj, this.handleWpObj);
					this.showObj.style.display = 'none';
				} else {
					alert('已经存在'+userName);
				}
		...
		showUser : function(type) {
			...
			if(this.showType == 1) {
				...
				$('showUser_'+type).className = 'normalbtn graybtn';
			}
		},
		append : function(uid, filtrate, form) {
			...
			if(this.showType == 1) {
				...
				if(form) {
					...
					liObj.innerHTML = '<a href="javascript:;" '+this._uname+'="' + this.dataSource[uid]['username'] + '" onmouseover="' + this.handleKey + '.mouseOverPrompter(this);" onclick="' + this.handleKey + '.selectUserName(this.getAttribute(\''+this._uname+'\'));$(\''+this._uname+'\').focus();" class="cl" id="prompter_' + uid + '">' + username + '</a>';
				}
		...
		initialize:function() {
			var instance = this;
			//初始化忽略列表中已存在的用户数据
			$j(".unamecard_msg").each(function(){
				var _userId = $j(this).attr("id");
				_userId = _userId.substring(3);
				instance.selectUser[_userId] = _userId;
			})
