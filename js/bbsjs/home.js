...
function resizeTx(obj){
	...
	if(obj.scrollHeight > 135){//textarea 最小高度135
...
function setpmstatus(form) {
	...
	if(requesturl) {
		ajaxget('home.php?mod=spacecp&ac=pm&op=setpmstatus' + requesturl, 'pm_status_reload', 'ajaxwaitid', '', 'none', 'markreadpm(\''+ markreadids.join(',') +'\')');
	}
}
...
function searchpostbyusername(keyword, srchuname) {
	window.location.href = 'index.php?m=bbsUser.user_search&srchtxt=' + keyword + '&srchuname=' + srchuname + '&searchsubmit=yes';
}